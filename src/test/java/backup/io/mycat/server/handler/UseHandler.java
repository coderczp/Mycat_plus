...
public final class UseHandler {
    ...
    public static void handle(String sql, ClientConn c, int offset) {
        ...
        if (!privileges.userExists(user, c.getHost())) {
            c.writeErrMessage((byte)1,ErrorCode.ER_ACCESS_DENIED_ERROR, "Access denied for user '" + c.getUser() + "'");
            return;
        }
