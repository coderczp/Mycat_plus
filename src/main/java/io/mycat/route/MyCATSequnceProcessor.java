...
public class MyCATSequnceProcessor {
    ...
    private static final DruidSequenceHandler sequenceHandler = new DruidSequenceHandler(
    ...
    public void executeSeq(SessionSQLPair pair) {
        try {
            ...
            RouteService router = instance.getRouterService();
            SystemConfig system = instance.getConfig().getSystem();
            RouteResultset rrs = router.route(system, pair.schema, pair.type, executeSql, charset, session.getSource());
            pair.session.execute(rrs, rrs.isSelectForUpdate() ? SQLHandler.Type.UPDATE : pair.type);
            ...
            pair.session.getSource().writeErrMessage((byte) 1, ErrorCode.ER_YES, "mycat sequnce err." + e);
            return;
        }
    }
}
