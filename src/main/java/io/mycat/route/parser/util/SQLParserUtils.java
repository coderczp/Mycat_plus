...
public class SQLParserUtils {
	 ...
	 * @return
	 */
	public static String getInsertTableName(String insertSql) {
	    String sql = insertSql.toUpperCase();
	    int start = sql.indexOf("INTO");
	    int end = sql.indexOf("(");
	    String tbl = sql.substring(start+4, end-1);
	    return tbl.replaceAll("`", "").trim();
	}
    ...
    public static void main(String[] args) {
        ...
        list.add(new String[] { "select * from B INNER JOIN C ON C.ID = (select G,H FROM I) INNER JOIN E ON 1=1",
                                "B,C,E,I" });
        list.add(new String[] { "select * from B INNER JOIN C ON C.ID = (select G,H FROM I ) INNER JOIN E ON 1=1",
                                "B,C,E,I" });
        list.add(new String[] { "select * from B INNER JOIN C ON C.ID = ( select G,H FROM I ) INNER JOIN E ON 1=1",
