...
public class MycatSchemaStatVisitor extends MySqlSchemaStatVisitor {
    ...
    public String getSchema() {
        ...
        int index = tbl.indexOf(".");
        if (index == -1) {
            return null;
        }
        
        String db = tbl.substring(0, index);
        if (db.indexOf("`") > -1) {
            db = db.replaceAll("`", "");
        }
    ...
    public String getTableWithOutDB() {
        String tbl = getCurrentTable();
        if (tbl == null) {
            return null;
        }
        ...
        String table = tbl.substring(index+1);
        if (table.indexOf("`") > -1) {
            ...
            switch (parentExpr.getOperator()) {
                ...
                case NotLessThan:
                    ...
                    if (sexpr instanceof SQLIdentifierExpr
                        || (sexpr instanceof SQLPropertyExpr
    ...
    public boolean visit(SQLBinaryOpExpr x) {
         ...
         *  这里设置好操作的是哪张表后,再进行判断.
         */
        String currenttable = x.getParent() == null ? null
            : (String) x.getParent().getAttribute(SchemaStatVisitor.ATTR_TABLE);
        if (currenttable != null) {
