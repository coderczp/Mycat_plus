...
public class RouteService {
    ...
    public RouteResultset route(SystemConfig system, SchemaConfig schema, int sqlType, String stmt, String charset,
        ...
        if (hintLength != -1) {
            rrs = handleHitsql(system, schema, sqlType, stmt, charset, sc, rrs, hintLength);
        } else {
            //stmt = stmt.trim();
            rrs = RouteStrategyFactory.getRouteStrategy().route(system, schema, sqlType, stmt, charset, sc,
    ...
    private RouteResultset handleHitsql(SystemConfig sysconf, SchemaConfig schema, int sqlType, String stmt,
                                        String charset, ClientConn sc, RouteResultset rrs,
                                        int hintLength) throws SQLSyntaxErrorException, SQLNonTransientException {
        ...
        if (endPos > 0) {
            ...
            if (firstSplitPos > 0) {
                ...
                if (hintSql.length() == 0) {
                    String msg = "comment int sql must meet:/*!mycat:type=value*/ or /*#mycat:type=value*/ or /*mycat:type=value*/: "
                                 + stmt;
                    LOGGER.warn(msg);
                ...
                if (hintHandler != null) {
                    if (hintHandler instanceof HintSQLHandler) {
                         ...
                         * fixed by zhuam
                         */
                        int hintSqlType = SimpleSqlParser.parse(hintSql) & 0xff;
                        rrs = hintHandler.route(sysconf, schema, sqlType, realSQL, charset, sc, tableId2DataNodeCache,
                            hintSql, hintSqlType, hintMap);
                    } else {
                        rrs = hintHandler.route(sysconf, schema, sqlType, realSQL, charset, sc, tableId2DataNodeCache,
                            hintSql, sqlType, hintMap);
                    }
                } else {
                ...
                LOGGER.warn(
                    "comment in sql must meet :/*!mycat:type=value*/ or /*#mycat:type=value*/ or /*mycat:type=value*/: "
                            + stmt);
                throw new SQLSyntaxErrorException(
                    "comment in sql must meet :/*!mcat:type=value*/ or /*#mycat:type=value*/ or /*mycat:type=value*/: "
                                                  + stmt);
            }
        }
        return rrs;
    ...
    private void checkMigrateRule(String schemal, RouteResultset rrs, int sqlType) throws SQLNonTransientException {
        ...
        if (rrs != null && rrs.getTables() != null) {
            ConcurrentMap<String, List<PartitionByCRC32PreSlot.Range>> tableRules = RouteCheckRule.migrateRuleMap
                .get(schemal.toUpperCase());
            if (tableRules != null) {
                for (String table : rrs.getTables()) {
                    ...
                    if (rangeList != null && !rangeList.isEmpty()) {
                        if (rrs.getNodes().length > 1 && isUpdate) {
                            throw new SQLNonTransientException("schema:" + schemal + ",table:" + table + ",sql:" + rrs
                                .getStatement() + " is not allowed,because table is migrate switching,please wait for a moment");
                        }
                            ...
                            for (RouteResultsetNode routeResultsetNode : routeResultsetNodes) {
                                ...
                                if (isUpdate && slot >= range.start && slot <= range.end) {
                                    throw new SQLNonTransientException(
                                        "schema:" + schemal + ",table:" + table + ",sql:" + rrs
                                            .getStatement() + " is not allowed,because table is migrate switching,please wait for a moment");
    ...
    private boolean isUpdateSql(int type) {
        return SQLHandler.Type.INSERT == type || SQLHandler.Type.UPDATE == type || SQLHandler.Type.DELETE == type
    ...
    public static int isHintSql(String sql) {
        ...
        if (sql.charAt(j++) == '/' && sql.charAt(j++) == '*') {
            ...
            if (sql.charAt(++j) == 'm' && sql.charAt(++j) == 'y' && sql.charAt(++j) == 'c' && sql.charAt(++j) == 'a'
                && sql.charAt(++j) == 't' && (sql.charAt(++j) == ':' || sql.charAt(j) == '#')) {
