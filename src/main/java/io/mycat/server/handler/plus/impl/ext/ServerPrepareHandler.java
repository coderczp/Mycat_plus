...
public class ServerPrepareHandler implements FrontendPrepareHandler {
    ...
    public ServerPrepareHandler(ClientConn source) {
        this.source = source;
        this.pstmtId = 0L;
	...
	public void sendLongData(byte[] data) {
		...
		if(pstmt != null) {
			...
			try {
				...
				source.writeErrMessage((byte)1,ErrorCode.ERR_FOUND_EXCEPION, e.getMessage());
			}
		}
	}
	...
	public void reset(byte[] data) {
		...
		if(pstmt != null) {
			...
			source.writeErrMessage((byte)1,ErrorCode.ERR_FOUND_EXCEPION, "can not reset prepare statement : " + pstmtForId.get(pstmtId));
		}
	} 
    ...
    public void execute(byte[] data) {
        ...
        if ((pstmt = pstmtForId.get(pstmtId)) == null) {
            source.writeErrMessage((byte)1,ErrorCode.ER_ERROR_WHEN_EXECUTING_COMMAND, "Unknown pstmtId when executing.");
        } else {
            ExecutePacket packet = new ExecutePacket(pstmt);
            try {
                packet.read(data, source.getCharset());
            } catch (UnsupportedEncodingException e) {
                source.writeErrMessage((byte)1,ErrorCode.ER_ERROR_WHEN_EXECUTING_COMMAND, e.getMessage());
                return;
            }
