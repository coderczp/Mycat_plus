...
/**
 * @author jeff.cao
 * @version 0.0.1, 2018年4月26日 上午11:46:51 
 */
public class ExplainHandler implements SQLHandler {
    ...
    @Override
    public int type() {
        return SQLHandler.Type.EXPLAIN;
    }
    ...
    public void handle(String srcSql, ClientConn c) {
        ...
        String sql = explain.getStatement().toString();
        
        RouteResultset rrs = getRouteResultset(c, sql);
        if (rrs == null) {
            return;
        }
    ...
    private static RouteResultset getRouteResultset(ClientConn c, String stmt) {
        String db = c.getSchema();
        Session session = c.getSession2();
        int sqlType = SimpleSqlParser.parse(stmt) & 0xff;
        MycatConfig config = session.getCurrentConfig();
        if (db == null) {
            db = SchemaUtil.detectDefaultDb(stmt, sqlType, session);
            if (db == null) {
        ...
        if (schema == null) {
            c.writeErrMessage((byte) 1, ErrorCode.ER_BAD_DB_ERROR, "Unknown database '" + db + "'");
            return null;
        }
        try {

            if (SQLHandler.Type.INSERT == sqlType && isMycatSeq(stmt, schema)) {
                c.writeErrMessage((byte) 1, ErrorCode.ER_PARSE_ERROR,
            ...
            c.writeErrMessage((byte) 1, ErrorCode.ER_PARSE_ERROR, msg == null ? e.getClass().getSimpleName() : msg);
            return null;
        }
    }
