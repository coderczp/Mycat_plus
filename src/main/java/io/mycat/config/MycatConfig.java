...
public class MycatConfig {
    ...
    private volatile ConfigInitializer           configIniter;

    public MycatConfig(ConfigLoader configLoader) {
        this.configIniter = new ConfigInitializer(configLoader, true);
        this.system = configIniter.getSystem();
        this.users = configIniter.getUsers();
        this.schemas = configIniter.getSchemas();
        this.dataHosts = configIniter.getDataHosts();

        this.dataNodes = configIniter.getDataNodes();
        for (PhysicalDBPool dbPool : dataHosts.values()) {
            dbPool.setSchemas(getDataNodeSchemasOfDataHost(dbPool.getHostName()));
        }

        this.firewall = configIniter.getFirewall();
        this.cluster = configIniter.getCluster();
    ...
    public ConfigInitializer getConfigIniter() {
        return configIniter;
    }
    ...
    public void setSocketParams(AbstractConnection con, boolean isFrontChannel) throws IOException {
        ...
        channel.setOption(StandardSocketOptions.TCP_NODELAY, soNoDelay);
    ...
    public void setClientSocketParams(ClientConn con) throws IOException {
        con.setPacketHeaderSize(system.getPacketHeaderSize());
        con.setMaxPacketSize(system.getMaxPacketSize());
        con.setIdleTimeout(system.getIdleTimeout());
        con.setCharset(system.getCharset());
     ...
     * @throws IOException
     */
    public  void configChannel(NetworkChannel channel) throws IOException {
        int sorcvbuf = system.getFrontsocketsorcvbuf();
        int sosndbuf = system.getFrontsocketsosndbuf();
        boolean soNoDelay = system.getFrontSocketNoDelay();
        channel.setOption(StandardSocketOptions.SO_RCVBUF, sorcvbuf);
        channel.setOption(StandardSocketOptions.SO_SNDBUF, sosndbuf);
        channel.setOption(StandardSocketOptions.TCP_NODELAY, soNoDelay);
        channel.setOption(StandardSocketOptions.SO_REUSEADDR, true);
