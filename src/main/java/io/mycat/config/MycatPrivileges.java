...
import io.mycat.config.model.UserPrivilegesConfig.TablePrivilege;
...
public class MycatPrivileges implements FrontendPrivileges {
    ...
    // 审计SQL权限
    @Override
    public boolean checkDmlPrivilege(String user, String schema, Session session) {
        ...
        UserPrivilegesConfig privilege = userConfig.getPrivilegesConfig();
        if (privilege == null || !privilege.isCheck()) {
            return true;
        }

        UserPrivilegesConfig.SchemaPrivilege schemaPrivilege = privilege.getSchemaPrivilege(schema);
        if (schemaPrivilege == null) {
            return true;
        }

        SqlContext ast = session.getSqlCtx();
        String sql = ast.getSql();

        //TODO 此处待优化，寻找更优SQL 解析器
        // 修复bug https://github.com/alibaba/druid/issues/1309
        //com.alibaba.druid.sql.parser.ParserException: syntax error, error in :'begin',expect END, actual EOF begin
        if (sql.length() == 5 && sql.equalsIgnoreCase("begin")) {
            return true;
        }

        //SQLStatementParser parser = new MycatStatementParser(sql);
        //SQLStatement stmt = parser.parseStatement();
        SQLStatement stmt = ast.getStmtHolder().getStmt();
        ...
        String tableName = ast.getVistor().getCurrentTable();
        if (tableName == null) {
            return true;
        }
        ...
        TablePrivilege tablePrivilege = schemaPrivilege.getTablePrivilege(tableName);
        if (tablePrivilege.getDml()[index] > 0) {
            return true;
        }

        ALARM.error(String.format("%s[sql=%s,user=%s]", Alarms.DML_ATTACK, sql, user));
        return false;
    }
