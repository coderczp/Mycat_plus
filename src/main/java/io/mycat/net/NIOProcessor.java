...
public final class NIOProcessor {
    ...
    private static final Logger                                  LOGGER          = LoggerFactory
        .getLogger("NIOProcessor");

    private final String                                         name;
    private final BufferPool                                     bufferPool;
    private final NameableExecutor                               executor;
    private final ConcurrentMap<Long, ClientConn>        frontends;
    ...
    public NIOProcessor(String name, BufferPool bufferPool, NameableExecutor executor) throws IOException {
        ...
        this.frontends = new ConcurrentHashMap<Long, ClientConn>();
    ...
    public int getWriteQueueSize() {
        int total = 0;
        for (ClientConn fron : frontends.values()) {
    ...
    public void addFrontend(ClientConn c) {
        this.frontends.put(c.getId(), c);
        this.frontendsLength.incrementAndGet();
    }

    public ConcurrentMap<Long, ClientConn> getFrontends() {
    ...
    private void frontendCheck() {
        Iterator<Entry<Long, ClientConn>> it = frontends.entrySet().iterator();
        while (it.hasNext()) {
            ClientConn c = it.next().getValue();
            // 删除空连接
            if (c == null) {
                it.remove();
    ...
    private void backendCheck() {
        long sqlTimeout = MycatServer.getInstance().getAnyVersionConfig().getSystem().getSqlExecuteTimeout() * 1000L;
