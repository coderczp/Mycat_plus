...
public class SingleNodeHandler implements ResponseHandler, Terminatable, LoadDataResponseHandler {
    ...
    public SingleNodeHandler(RouteResultset rrs, Session session) {
        ...
        netInBytes += statement.getBytes().length;

        ClientConn source = session.getSource();
        String schema = source.getSchema();
        if (schema != null && SQLHandler.Type.SHOW == rrs.getSqlType()) {
            MycatConfig currentConfig = session.getCurrentConfig();
            SchemaConfig schemaConfig = currentConfig.getSchemas().get(schema);
            int type = ServerParseShow.tableCheck(statement, 0);
            ...
            if (isDefaultNodeShowTable) {
                shardingTablesSet = ShowTables.getTableSet(source, statement);

            } else if (isDefaultNodeShowFullTable) {
                shardingTablesSet = ShowFullTables.getTableSet(source, statement);
            }
        }
    ...
    private void _execute(BackendConnection conn) {
        ...
        try {
            ...
            ClientConn source = session.getSource();
            conn.execute(node, source, source.isAutocommit());
        } catch (Exception e1) {
    ...
    public void okResponse(byte[] data, BackendConnection conn) {
        ...
        if (executeResponse) {
            ...
            QueryResult queryResult = new QueryResult(session.getSource().getUser(), rrs.getSqlType(),
                rrs.getStatement(), affectedRows, netInBytes, netOutBytes, startTime, System.currentTimeMillis(), 0);
    ...
    public void rowEofResponse(byte[] eof, BackendConnection conn) {
        ...
        MycatConfig currentConfig = session.getCurrentConfig();
        int resultSize = source.getWriteQueue().size() * currentConfig.getSystem().getBufferPoolPageSize();
