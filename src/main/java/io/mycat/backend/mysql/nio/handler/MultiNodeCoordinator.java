            ...
            if (conn != null) {
                ...
                if (conn instanceof MySQLConnection) {
                    ...
                    if (mysqlCon.getXaStatus() == TxState.TX_STARTED_STATE) {
                        //recovery Log
                        participantLogEntry[started] = new ParticipantLogEntry(xaTxId, conn.getHost(), 0,
                            conn.getSchema(), ((MySQLConnection) conn).getXaStatus());
    ...
    public void errorResponse(byte[] err, BackendConnection conn) {
        ...
        if (conn instanceof MySQLConnection) {
            ...
            if (xaTxId != null) {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("Replay Commit execute the cmd :" + cmd + ",current host:" + mysqlCon.getHost() + ":"
                                 + mysqlCon.getPort());
                }
            ...
            switch (mysqlCon.getXaStatus()) {
                case TxState.TX_STARTED_STATE:
                    ...
                    if (mysqlCon.batchCmdFinished()) {
                        ...
                        if (LOGGER.isDebugEnabled()) {
                            LOGGER.debug("Start execute the cmd :" + cmd + ",current host:" + mysqlCon.getHost() + ":"
                                         + mysqlCon.getPort());
                        }
                        //recovery log
