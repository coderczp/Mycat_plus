...
public class MySQLConnection extends BackendAIOConnection {
    ...
    public MySQLConnection(NetworkChannel channel, boolean fromSlaveDB, int txIsolation) {
        super(channel);
    ...
    public void execute(RouteResultsetNode rrn, ClientConn sc,
        ...
        String xaTXID = null;
        String xaTXID2 = sc.getSession2().getXaTXID();
        if (xaTXID2 != null) {
            //xaTXID = sc.getSession2().getXaTXID()+",'"+getSchema()+"'";
            xaTXID = new StringBuilder(xaTXID2).append(",'").append(getSchema()).append("'").toString();
        }
    ...
    public void query(String query) throws UnsupportedEncodingException {
        RouteResultsetNode rrn = new RouteResultsetNode("default", SQLHandler.Type.SELECT, query);
    ...
    public String toString() {
        return "MySQLConnection [id=" + id + ", lastTime=" + lastTime + ", user=" + user + ", schema=" + schema
               ...
               + ", statusSync=" + statusSync + ", writeQueue=" + this.getWriteQueue().size() + ", modifiedSQLExecuted="
               + modifiedSQLExecuted + "]";
    }
